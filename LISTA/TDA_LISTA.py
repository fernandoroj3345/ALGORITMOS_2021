...
class Nodo_Lista():# creo el nodo para enlazar la lista
...
def criterio(dato, campo=None):
    '''Determino el campo por el cual debo comparar el dato'''
    dic = {}
    if(hasattr(dato, '__dict__')):
        dic = dato.__dict__
    if campo is None or campo not in dic:#comparo por dato simple
        return dato
    else:
        return dic[campo]
...
def busqueda_lista(lista, buscado, campo=None):#busco por criterio.
...
def busqueda_lista_vec(lista, buscado, campo=0):#busco por informacion
    '''Devuelve el elemento buscado en una lista, si es que se encuentra'''
    aux = lista.inicio
